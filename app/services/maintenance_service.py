...
def create_maintenance(db: Session, maintenance_data: MaintenanceCreate):
    maintenance = maintenance_repository.create_maintenance(
        db,
        maintenance_data
    )

    next_service_due = (
        maintenance.last_service_mileage
        + maintenance.service_interval_km
    )
...
def get_all_maintenance(db: Session):
    maintenance_records = (
        maintenance_repository.get_all_maintenance(db)
    )
